...
```cpp
LD2450radar radar(Serial1, 16, 17, 256000, 10, 0.1f, 6.0f);

```
#### Точное отслеживание
```cpp
LD2450radar radar(Serial1, 16, 17, 256000, 3, 0.3f, 4.0f);
```
#### Быстрое реагирование
```cpp
LD2450radar radar(Serial1, 16, 17, 256000, 5, 0.5f, 8.0f);
```
...
```cpp int getID(int n) - ID n-й активной цели ```
```cpp float getDistance(int n) - дистанция (метры) ```
```cpp float getAngle(int n) - угол (градусы)```
